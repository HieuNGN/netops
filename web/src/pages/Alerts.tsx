...
export function Alerts() {
  ...
  const initialTab: "active" | "rules" | "integrations" | "windows" =
    tabParam === "rules" ||
  ...
  const [activeTab, setActiveTab] = useState<
    "active" | "rules" | "integrations" | "windows"
  >(initialTab);
  ...
  const derivedTab: "active" | "rules" | "integrations" | "windows" =
    focusChannel ? "rules" : activeTab;
  ...
  const switchTab = (t: "active" | "rules" | "integrations" | "windows") => {
    setActiveTab(t);
    setSearchParams(
      (prev) => {
    ...
    onError: (e: any) => {
      toast.error(
        e?.response?.data?.detail ||
          "Delete blocked — alert rules may reference this",
      );
    },
  });

  const testInteg = useMutation({
    mutationFn: (id: string) => integrationsApi.test(id),
    onSuccess: (resp) => {
      if (resp.data.sent) toast.success("Test message sent");
      else toast.error("Test failed — check integration config");
    },
    onError: (e: any) => {
  ...
  function resetIntegForm() {
    ...
    setIntegType("telegram");
    setIntegName("");
    setIntegSecrets({});
  }

  function startEditInteg(integ: any) {
    setEditingInteg(integ);
    setIntegType(integ.type);
    setIntegName(integ.name);
    setIntegSecrets({ ...(integ.secrets_json || {}) });
    setShowIntegForm(true);
  }

  function handleIntegSubmit(e: React.FormEvent) {
    e.preventDefault();
    const cleaned: Record<string, string> = {};
    Object.entries(integSecrets).forEach(([k, v]) => {
      if (v !== "" && v != null) cleaned[k] = v;
    });
    if (editingInteg) {
      updateInteg.mutate({
        id: editingInteg.id,
        data: { name: integName, secrets_json: cleaned },
      });
    } else {
      createInteg.mutate({
        type: integType,
        name: integName,
        secrets_json: cleaned,
      });
    }
  }
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        handleChannelChange(e.target.value as IntegrationType)
                      }
                      className="w-full px-3 py-2 border border-input dark:border-input bg-card text-foreground rounded-sm focus:ring-1 focus:ring-destructive"
                    >
                      {CHANNELS.map((c) => (
                    ...
                    <div className="bg-surface-subtle p-4 rounded-sm space-y-3">
                        ...
                        <div>
                          ...
                          {available.length === 0 && (
                            <p className="text-xs text-muted-foreground mt-1">
                              No {channel.label} integrations yet. Add one in
                      ...
                      {newAlert.integration_id && (
                        <p className="text-xs text-muted-foreground">
                          Credentials pulled from the selected integration.
                          Per-rule overrides below will take precedence.
                        </p>
                      )}
                    </div>
                  );
                ...
                <div className="flex justify-end space-x-2">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-thinkpad-red text-white rounded-sm hover:bg-thinkpad-red-hover disabled:opacity-50"
                  >
                    {editingAlertId
          ...
          <div className="bg-card rounded-sm shadow-sm border border-border overflow-hidden">
            ...
            <div className="divide-y divide-border">
              {isLoading ? (
                ...
                alerts.map((alert) => {
                  const linkedInteg = allIntegrations.find(
                    (i) => i.id === alert.integration_id,
                  );
                  return (
                    <div
                      key={alert.id}
                      className="px-6 py-4 hover:bg-muted dark:hover:bg-muted"
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <button
                            onClick={() => {
                              if (
                                confirm(`Delete alert rule "${alert.name}"?`)
                              ) {
              ...
              activeAlerts.map((alert) => (
                <div
                  ...
                  className={`px-6 py-4 flex items-center justify-between border-l-4 ${
                    alert.status === "firing"
                      ? "bg-badge-destructive-bg border-l-ibm-red"
                      : "bg-badge-warning-bg border-l-ibm-yellow"
                  }`}
                >
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">
                        ...
                        Fired{" "}
                        {new Date(alert.fired_at * 1000).toLocaleTimeString()}
                      </p>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="md:col-span-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setNewWindow({ ...newWindow, name: e.target.value })
                      }
                      className="w-full px-3 py-2 border border-input dark:border-input bg-card text-foreground rounded-sm focus:ring-1 focus:ring-destructive"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      value={integType}
                      onChange={(e) => {
                        setIntegType(e.target.value as IntegrationType);
                        setIntegSecrets({});
                      }}
                      disabled={!!editingInteg}
                      className="w-full px-3 py-2 border border-input bg-card text-foreground rounded-sm focus:ring-1 focus:ring-thinkpad-red disabled:opacity-60"
                    >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      required
                      placeholder="Ops Bot"
                      className="w-full px-3 py-2 border border-input bg-card text-foreground rounded-sm focus:ring-1 focus:ring-thinkpad-red"
                    />
                  </div>
                </div>
                {getFieldsForType(integType).map((field) => {
                  const hint = HINTS[integType]?.[field.key];
                  return (
                    <div key={field.key}>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-1 shrink-0">
                        <button
                          onClick={() => testInteg.mutate(integ.id)}
                          disabled={testInteg.isPending}
                          className="p-2 text-muted-foreground hover:bg-surface-hover rounded-sm disabled:opacity-50"
                          title="Send test message"
                        >
                          <Send className="h-4 w-4" />
                        </button>
                        <button
                          onClick={() => startEditInteg(integ)}
                          className="p-2 text-muted-foreground hover:bg-surface-hover rounded-sm"
                          title="Edit"
                        >
                          <Pencil className="h-4 w-4" />
