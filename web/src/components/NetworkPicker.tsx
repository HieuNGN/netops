...
export function NetworkPicker() {
  ...
  if (isLoading) {
    return <div className="text-sm text-muted-foreground">Loading networks...</div>;
  }
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="bg-background border border-border p-3 rounded-sm space-y-2">
          <input
            ...
            className="w-full px-2 py-1.5 text-sm border border-input dark:border-input bg-card text-foreground rounded-sm"
            required
          />
          <input
            type="text"
